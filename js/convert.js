...
export function nrToLog(nr) {
  ...
  if ((nr >= 10 && nr <= 14) || (nr >= 20 && nr <= 24)) {
    ...
    if (nr < 20) {
      const x = Math.random();
      if (x < 0.2) msg = `The quick ${name} butterfly slips through your fingers, leaving only a fleeting trace of its delicate touch.`;
      else if (x < 0.4) msg = `You strive to capture the ethereal beauty of a ${name} butterfly, but it eludes your grasp.`;
